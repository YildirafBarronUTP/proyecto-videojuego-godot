...
func es_zona_segura(x: int, y: int) -> bool:
	if (x == 1 and y == 1) or (x == 2 and y == 1) or (x == 1 and y == 2): 
		return true
	if (x == columnas - 2 and y == filas - 2) or (x == columnas - 3 and y == filas - 2) or (x == columnas - 2 and y == filas - 3): 
		return true
	return false

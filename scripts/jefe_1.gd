...
@export var limite_tiempo_atascado: float = 0.15 # Se redujo el tiempo de reacción para que destrabe más rápido
...
func _physics_process(delta: float) -> void:
	...
	if not camino_actual.is_empty():
		...
		if timer_impulso > 0.0:
			...
			# Evaluamos el desplazamiento únicamente mientras el jefe debería estarse moviendo físicamente
			...
			velocity = Vector2.ZERO
			tiempo_atascado = 0.0 # Se limpia al apagarse el motor para no acumular falsos positivos
